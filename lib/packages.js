...
function normalizeAddonAssignments(rawAddons, rawByPhoto, rawList) {
  ...
  function push(key, id, filename, qtyRaw, label, index) {
    ...
    const photoLabel = truncateStr(label, 160)
    ...
    assignments.push({
      ...
      index: typeof index === "number" && index >= 0 ? index : undefined,
    });
  }
        ...
        value.forEach((row) => {
          if (row && typeof row === "object") {
            push(
              ...
              row.qty,
              row.label,
              typeof row.index === "number" ? row.index : undefined
            );
        ...
        for (const [photoId, q] of Object.entries(value)) {
          if (q && typeof q === "object") {
            push(
              key,
              photoId || q.id,
              q.filename || q.name || photoId,
              q.qty,
              q.label,
              typeof q.index === "number" ? q.index : undefined
            );
          } else {
 ...
 * Example:
 * Photo 1 — smile.jpg: Extra 8×10 ×2 ($24); Extra 5×7 ×1 ($6)
 * Photo 2 — pose.jpg: Extra 8×10 ×1 ($12)
 */
  ...
  assignments.forEach((a) => {
    const label = a.label
      || (typeof a.index === "number" && a.index >= 0
        ? `Photo ${a.index + 1} — ${a.filename || a.id || "Photo"}`
        : `Photo — ${a.filename || a.id || "Photo"}`);
